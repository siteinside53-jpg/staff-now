...
function checkTaskNow() {
  ...
  // 4. Κλειστή μακέτα σημαίνει ότι σβήνεται και ό,τι έχει ήδη αποθηκευτεί.
  //
  //    ΤΙ ΠΗΓΕ ΣΤΡΑΒΑ ΚΑΙ ΕΦΤΑΣΕ ΣΕ ΧΡΗΣΤΗ: ο διακόπτης άδειασε τα δείγματα
  //    στο χτίσιμο, οπότε η νέα έκδοση δεν φτιάχνει ψεύτικες μικροδουλειές.
  //    Όποιος όμως είχε ανοίξει το TaskNow σε παλιότερη έκδοση κρατούσε τις 16
  //    στον browser του, και επειδή η επαναφορά διάβαζε την αποθήκευση χωρίς
  //    έλεγχο, οι ψεύτικες δουλειές ξαναέβγαιναν στο staffnow.gr — μέσα στον
  //    λογαριασμό, δίπλα σε αληθινές αγγελίες.
  const storeFile = 'apps/web/src/components/tasknow/mock-store.ts';
  {
    const src = read(storeFile);
    const hydrateBody = src.slice(src.indexOf('function hydrate()'));
    if (!/SAMPLE_TASKS\.length === 0[\s\S]{0,240}removeItem/.test(hydrateBody)) {
      fail(
        'Η κλειστή μακέτα δεν σβήνει την παλιά αποθήκευση',
        'Στη mock-store.ts, η hydrate() πρέπει να πετάει την αποθηκευμένη ' +
          'κατάσταση όταν τα δείγματα είναι άδεια. Αλλιώς όποιος είδε τη μακέτα ' +
          'σε παλιότερη έκδοση συνεχίζει να βλέπει ψεύτικες μικροδουλειές στο live.'
      );
    } else {
      ok('TaskNow: κλειστή μακέτα σβήνει και τα ψεύτικα που είχαν μείνει στον browser');
    }
  }

  // 5. Η ενότητα μέσα στον λογαριασμό λέει την αλήθεια όσο δεν έχει ανοίξει.
  const hubPage = 'apps/web/src/app/dashboard/tasknow/page.tsx';
  {
    const src = stripComments(read(hubPage));
    if (!/TASKNOW_DEMO/.test(src)) {
      fail(
        'Ο πίνακας δείχνει τη μακέτα του TaskNow χωρίς διακόπτη',
        'Το /dashboard/tasknow πρέπει να δείχνει «ανοίγει σύντομα» όσο η μακέτα ' +
          'είναι κλειστή. Αλλιώς ο συνδεδεμένος χρήστης πατάει «ανέβασε μικροδουλειά» ' +
          'και η δουλειά δεν πάει πουθενά.'
      );
    } else {
      ok('TaskNow: μέσα στον λογαριασμό δεν παριστάνει ότι λειτουργεί');
    }
  }
}
