...
function checkVideoCalls() {
  ...
  */
  ...
  /*
    (7) ΤΟ ΚΕΝΤΡΟ ΚΛΗΣΕΩΝ ΠΡΕΠΕΙ ΝΑ ΕΙΝΑΙ ΚΡΕΜΑΣΜΕΝΟ ΣΤΟΝ ΛΟΓΑΡΙΑΣΜΟ.

    Αυτό είναι το μοναδικό σημείο όπου ζει το «με καλεί κανείς;». Αν φύγει —
    και ΕΧΕΙ ΞΑΝΑΦΥΓΕΙ κατά λάθος — όλα τα υπόλοιπα συνεχίζουν να δουλεύουν
    τέλεια: ο καλών βλέπει «Κλήση…», ο server γράφει κανονικά τη γραμμή, καμία
    δοκιμή δεν σκάει. Απλώς δεν χτυπάει ποτέ τίποτα στον άλλον.

    Είναι ακριβώς το σχήμα «το φτιάξαμε τρεις φορές και ξανάσπασε»: η βλάβη
    είναι αόρατη από παντού εκτός από μια αληθινή κλήση με δύο λογαριασμούς.

    Ελέγχουμε και το `enabled`: με σταθερό false θα ήταν σαν να μην υπάρχει.
  */
  {
    const layout = stripComments(read('apps/web/src/app/dashboard/layout.tsx'));
    const mount = /<CallCenter\s+enabled=\{([^}]+)\}/.exec(layout);
    if (!/from\s+'@\/components\/video\/call-center'/.test(layout) || !mount) {
      fail(
        'Το κέντρο κλήσεων δεν είναι κρεμασμένο στον λογαριασμό',
        'Το dashboard/layout.tsx πρέπει να τυλίγει τις σελίδες σε <CallCenter enabled={…}>. ' +
          'Χωρίς αυτό κανείς δεν ρωτάει «με καλεί κανείς;» και το τηλέφωνο δεν ' +
          'χτυπάει ΠΟΤΕ — ενώ όλα τα άλλα δείχνουν να δουλεύουν.'
      );
    } else if (/^\s*false\s*$/.test(mount[1])) {
      fail(
        'Το κέντρο κλήσεων είναι κλειστό',
        'Το <CallCenter enabled={…}> είναι μόνιμα false, άρα δεν ρωτάει ποτέ αν ' +
          'υπάρχει εισερχόμενη κλήση.'
      );
    } else {
      ok('Βιντεοκλήση: το κέντρο κλήσεων είναι κρεμασμένο σε όλες τις σελίδες του λογαριασμού');
    }
  }
}
...
/*
  Η ΠΛΗΡΩΜΕΝΗ ΠΡΟΩΘΗΣΗ ΠΡΕΠΕΙ ΝΑ ΦΑΙΝΕΤΑΙ ΕΚΕΙ ΠΟΥ ΤΗΝ ΑΓΟΡΑΖΟΥΝ.

  Η επιχείρηση δίνει 5 μονάδες για 7 μέρες προβολής. Το μεγαλύτερο κοινό μιας
  αγγελίας είναι οι επισκέπτες ΧΩΡΙΣ λογαριασμό — δηλαδή η «Βρες εργασία» και η
  αρχική, που τρέφονται από το /public/jobs.

  Δύο ξεχωριστά σημεία μπορούν να ακυρώσουν την πληρωμή, και το καθένα μόνο του
  αρκεί:

    • ο server να μη γνωρίζει καν την προώθηση (ήταν σκέτο ORDER BY ημερομηνία)·
    • η λίστα στη σελίδα να ξαναταξινομεί μόνη της κατά ημερομηνία, σβήνοντας
      ό,τι έστειλε ο server.

  Και τα δύο ήταν αληθινά μαζί. Η βλάβη ήταν αόρατη σε όποιον δοκίμαζε
  συνδεδεμένος, γιατί η συνδεδεμένη λίστα το έκανε ήδη σωστά.
*/
function checkBoostVisible() {
  const idx = stripComments(read('apps/api/src/index.ts'));
  const start = idx.indexOf("app.get('/public/jobs'");
  const block = start < 0 ? '' : idx.slice(start, start + 2200);

  if (!/job_boosts/.test(block) || !/ORDER BY[^`]*is_boosted/.test(block)) {
    fail(
      'Η δημόσια λίστα αγγελιών αγνοεί την πληρωμένη προώθηση',
      'Το /public/jobs (index.ts) πρέπει να ενώνεται με τα job_boosts σε ισχύ και ' +
        'να ταξινομεί με is_boosted πρώτα. Αλλιώς η επιχείρηση πληρώνει και δεν ' +
        'παίρνει τίποτα στη «Βρες εργασία» και στην αρχική.'
    );
  } else {
    ok('Προώθηση: ο server ανεβάζει τις πληρωμένες αγγελίες στη δημόσια λίστα');
  }

  const list = stripComments(read('apps/web/src/components/marketing/public-jobs-list.tsx'));
  if (/\.sort\(\s*\(a,\s*b\)\s*=>\s*b\.createdAtMs\s*-\s*a\.createdAtMs\s*\)/.test(list)) {
    fail(
      'Η «Βρες εργασία» σβήνει τη σειρά της προώθησης',
      'Η λίστα ταξινομεί σκέτα κατά ημερομηνία, οπότε η πληρωμένη αγγελία ξαναπέφτει ' +
        'στη θέση της ημερομηνίας της. Η ταξινόμηση πρέπει να βάζει πρώτα το boosted.'
    );
  } else if (!/boosted/.test(list)) {
    fail(
      'Η «Βρες εργασία» δεν ξέρει τι είναι προωθημένη αγγελία',
      'Η λίστα πρέπει να διαβάζει το is_boosted από τον server και να το τιμά στη σειρά.'
    );
  } else {
    ok('Προώθηση: η «Βρες εργασία» κρατάει τις πληρωμένες αγγελίες πάνω');
  }
}

/*
  ΚΑΜΙΑ ΨΕΥΤΙΚΗ ΕΠΑΛΗΘΕΥΣΗ ΣΤΟ TASKNOW.

  Η οθόνη «κάνε προσφορά» ζητούσε κινητό, έλεγε ότι έστειλε SMS και δεχόταν
  ΟΠΟΙΟΝΔΗΠΟΤΕ τετραψήφιο αριθμό, γράφοντας «το κινητό σου επαληθεύτηκε».
  Δεν ρωτούσε ΠΟΤΕ τον server. Ήταν λείψανο της μακέτας που έμεινε μέσα στη
  ζωντανή ροή, αφού όλα τα υπόλοιπα γύρω του έγιναν αληθινά.

  Είναι το χειρότερο είδος σφάλματος για αυτό το προϊόν: δεν κρασάρει, δεν
  φαίνεται πουθενά, και δίνει στον χρήστη σήμα εμπιστοσύνης που δεν υπάρχει.

  Ο έλεγχος κοιτάει τρία πράγματα μαζί, γιατί το καθένα μόνο του παρακάμπτεται:
  ότι μιλάει στον server, ότι ο κωδικός είναι 6ψήφιος (ο πραγματικός), και ότι
  το πράσινο «επαληθεύτηκε» δεν είναι σταθερά γραμμένο στην οθόνη.
*/
function checkTaskNowPhoneReal() {
  const src = stripComments(read('apps/web/src/components/tasknow/offer-modal.tsx'));

  // Ψάχνουμε την ΚΛΗΣΗ, όχι απλώς το όνομα: ένα «auth.NOPE_confirmPhone» θα
  // περνούσε τον χαλαρό έλεγχο ενώ δεν καλεί τίποτα.
  if (!/auth\.sendPhoneCode\s*\(/.test(src) || !/auth\.confirmPhone\s*\(/.test(src)) {
    fail(
      'Η επαλήθευση κινητού στο TaskNow δεν ρωτάει τον server',
      'Το offer-modal.tsx πρέπει να καλεί auth.sendPhoneCode και auth.confirmPhone, ' +
        'όπως ακριβώς η σελίδα επαλήθευσης του λογαριασμού. Χωρίς αυτά δεχόμαστε ' +
        'οποιονδήποτε κωδικό και λέμε ψέματα στον χρήστη.'
    );
  } else {
    ok('TaskNow: η επαλήθευση κινητού περνάει από τον server');
  }

  if (/\\d\{4\}/.test(src) || /maxLength=\{4\}/.test(src)) {
    fail(
      'Ο κωδικός επαλήθευσης στο TaskNow είναι 4ψήφιος',
      'Ο πραγματικός κωδικός του server είναι ΕΞΑΨΗΦΙΟΣ. Τετραψήφιος έλεγχος ' +
        'σημαίνει ότι κάποιος ξαναέβαλε τον έλεγχο της μακέτας.'
    );
  } else {
    ok('TaskNow: ο κωδικός επαλήθευσης είναι 6ψήφιος, όπως τον στέλνει ο server');
  }

  // Το ✓ πρέπει να εξαρτάται από κάτι· σκέτο μέσα στο JSX σημαίνει «πάντα».
  const badge = /Το κινητό σου επαληθεύτηκε/.test(src);
  if (badge && !/smsAvailable\s*\?/.test(src)) {
    fail(
      'Το «επαληθεύτηκε» εμφανίζεται πάντα',
      'Το πράσινο σήμα στο offer-modal.tsx πρέπει να μπαίνει μόνο όταν έγινε ' +
        'πραγματική επαλήθευση. Χωρίς πάροχο SMS το σωστό σήμα είναι «δηλωμένο».'
    );
  } else {
    ok('TaskNow: το «επαληθεύτηκε» μπαίνει μόνο όταν όντως επαληθεύτηκε');
  }
}

/*
  Ο ΟΔΗΓΟΣ ΤΟΠΟΘΕΣΙΑΣ ΔΕΝ ΑΝΟΙΓΕΙ ΜΟΝΟΣ ΤΟΥ.

  Στο ταμπλό των μικροδουλειών, ο έλεγχος άδειας που τρέχει με το φόρτωμα της
  σελίδας άνοιγε παράθυρο «ο browser δεν μας δίνει την τοποθεσία» σε όποιον
  είχε κάποτε αρνηθεί — χωρίς να έχει πατήσει τίποτα. Έκρυβε τη λίστα, έμοιαζε
  με βλάβη, και έκανε τον χρήστη να νομίζει ότι το κουμπί «Κοντά μου» δεν
  δουλεύει ενώ δεν το είχε αγγίξει.

  Ο οδηγός πρέπει να ανοίγει ΜΟΝΟ από ενέργεια του χρήστη — δηλαδή από την
  αποτυχία του getCurrentPosition, όχι από το permissions.query.
*/
function checkLocationGuideNotAuto() {
  const src = stripComments(read('apps/web/src/components/tasknow/task-feed.tsx'));
  const start = src.indexOf('navigator.permissions');
  if (start < 0) {
    ok('Μικροδουλειές: δεν υπάρχει αυτόματος έλεγχος άδειας τοποθεσίας');
    return;
  }
  // Το κομμάτι που τρέχει με το φόρτωμα, μέχρι το τέλος της αλυσίδας.
  const end = src.indexOf('.catch(', start);
  const block = src.slice(start, end > start ? end : start + 900);
  if (/setGuide\s*\(/.test(block)) {
    fail(
      'Ο οδηγός τοποθεσίας πετάγεται μόνος του με το φόρτωμα',
      'Στο task-feed.tsx, ο έλεγχος permissions.query δεν επιτρέπεται να καλεί ' +
        'setGuide. Ο οδηγός ανοίγει μόνο όταν ο χρήστης πατήσει «Κοντά μου» και ' +
        'αποτύχει το getCurrentPosition. Αλλιώς εμφανίζεται παράθυρο σφάλματος σε ' +
        'κάποιον που δεν ζήτησε ποτέ τοποθεσία.'
    );
  } else {
    ok('Μικροδουλειές: ο οδηγός τοποθεσίας ανοίγει μόνο όταν το ζητήσει ο χρήστης');
  }

  // Η διέξοδος όταν ο browser αρνείται: να μπορεί να γράψει τη διεύθυνσή του.
  if (!/ή γράψε τη διεύθυνσή σου/.test(src)) {
    fail(
      'Δεν υπάρχει τρόπος να γράψει ο χρήστης τη διεύθυνσή του',
      'Μια άρνηση τοποθεσίας ΔΕΝ ξαναρωτιέται ποτέ από τον browser. Χωρίς πεδίο ' +
        'διεύθυνσης στο ταμπλό των μικροδουλειών, ο χρήστης μένει κλειδωμένος στο ' +
        '«Όλη η Θεσσαλονίκη» χωρίς καμία διέξοδο.'
    );
  } else {
    ok('Μικροδουλειές: υπάρχει «γράψε τη διεύθυνσή σου» όταν αρνείται ο browser');
  }
}
