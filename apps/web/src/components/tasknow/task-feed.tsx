  ...
  function locate(userAsked = true) {
    ...
    */
    ...
    navigator.geolocation.getCurrentPosition(onOk, () => {
      ...
      /*
        ΤΟ ΧΡΟΝΟΜΕΤΡΟ ΤΡΕΧΕΙ ΟΣΟ Ο ΧΡΗΣΤΗΣ ΔΙΑΒΑΖΕΙ ΤΗΝ ΕΡΩΤΗΣΗ.

        Όταν ο browser δείχνει «θέλει να χρησιμοποιήσει την τοποθεσία σου», αυτό
        εδώ ήδη μετράει. Με 12 δευτερόλεπτα, όποιος διάβαζε την ερώτηση αντί να
        πατήσει αμέσως — δηλαδή σχεδόν κάθε πρωτάρης, ειδικά σε κινητό — έβλεπε
        «απέτυχε» ενώ ήταν έτοιμος να πει ναι. Και τότε ο browser θυμάται την
        άρνηση και ΔΕΝ ξαναρωτάει ποτέ.

        Μισό λεπτό: αρκετό για να διαβαστεί και να απαντηθεί χωρίς βιασύνη.
      */
