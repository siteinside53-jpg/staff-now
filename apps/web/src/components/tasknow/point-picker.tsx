  ...
  function pickMyLocation() {
    ...
    watchId = navigator.geolocation.watchPosition(
      ...
      (err) => {
        finish();
        /*
          ΔΕΝ ΑΦΗΝΟΥΜΕ ΤΟΝ ΧΡΗΣΤΗ ΣΤΟ ΚΕΝΟ.

          Εδώ βγαίναμε μόνο με μήνυμα λάθους και οδηγίες για ρυθμίσεις. Όποιος
          είχε πει «όχι» μία φορά — ή δεν ήξερε πού να ψάξει — έμενε χωρίς
          πινέζα και εγκατέλειπε το ανέβασμα.

          Ο browser ΔΕΝ ξαναρωτάει μετά από άρνηση· είναι κανόνας δικός του και
          δεν παρακάμπτεται. Βάζουμε όμως την πινέζα κατά προσέγγιση, από τη
          σύνδεση, και το λέμε καθαρά: ο χάρτης είναι από κάτω και σύρεται.
        */
        void (async () => {
          try {
      ...
      // Μισό λεπτό: το χρονόμετρο τρέχει ΚΑΙ όσο ο χρήστης διαβάζει την ερώτηση
      // του browser. Με λίγα δευτερόλεπτα, όποιος δεν πατούσε αμέσως έβλεπε
      // «απέτυχε» — και μετά ο browser δεν ξαναρωτάει ποτέ.
