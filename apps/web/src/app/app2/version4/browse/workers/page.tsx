...
export default function BrowseWorkers() {
  ...
  const [stats, setStats] = useState({ total: 50000, online: 1847, today: 0 });
    ...
    (async () => {
      try {
        ...
        const mapped: Worker[] = items.map((w: any, i: number) => ({
          ...
          city: w.city || '',
          region: w.region || '',
          exp: w.years_of_experience ? `${w.years_of_experience} χρόνια` : 'Νέος/α',
          photo: w.photo_url,
        ...
        setWorkers(mapped);
        setStats({
          total: mapped.length + 50000,
          online: 1847 + Math.floor(Math.random() * 200),
          today: Math.floor(Math.random() * 150) + 230,
        });
      } catch {}
    })();
  }, []);
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowSignupGate(null)}>
          <div className="w-full max-w-md bg-white rounded-t-3xl p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-6 space-y-2">
              <button
                onClick={() => router.push('/app2/version4/signup?role=business')}
