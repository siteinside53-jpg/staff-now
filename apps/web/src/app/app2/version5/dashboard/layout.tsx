...
export default function DashV4Layout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.replace('/app2/version5/login');
      setLoading(false);
      return;
    }
    (async () => {
  ...
  // Order: Αρχική · Matches · Εύρεση · Μηνύματα · Προφίλ
  // Icons remain the v5 originals; only the matches heart was replaced with
  // handshake at the user's request.
  const tabs = isBusiness
    ? [
        { href: '/app2/version5/dashboard', label: 'Αρχική', icon: '🏠' },
        { href: '/app2/version5/dashboard/matches', label: 'Matches', icon: '🤝' },
        { href: '/app2/version5/dashboard/swipe', label: 'Εύρεση', icon: '🔍' },
        { href: '/app2/version5/dashboard/chat', label: 'Μηνύματα', icon: '💬' },
        { href: '/app2/version5/dashboard/profile', label: 'Προφίλ', icon: '👤' },
      ]
    : [
        { href: '/app2/version5/dashboard', label: 'Αρχική', icon: '🏠' },
        { href: '/app2/version5/dashboard/matches', label: 'Matches', icon: '🤝' },
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-gray-50 overflow-hidden">
      ...
      <nav className="flex-shrink-0 bg-white border-t border-gray-200 flex items-center justify-around" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {tabs.map((tab) => {
          const active = pathname === tab.href || (tab.href !== '/app2/version5/dashboard' && pathname.startsWith(tab.href));
          return (
