...
/**
 * Business matches — workers who matched with this business's jobs.
 * Mirrors /worker/matches (3 tabs: deals / σε σκέφτηκαν / σε ενδιαφέρουν).
 */
...
export default function BusinessMatches() {
  ...
  return (
    <>
      <AppBar title="Matches" subtitle="Εργαζόμενοι που ταίριαξαν με τις αγγελίες σου" />
      ...
      <Body>
        {loading ? (
          ...
          <EmptyState
            ...
            description="Κάνε swipe σε υποψήφιους ή δημοσίευσε αγγελίες για να ξεκινήσουν τα matches."
          />
        ) : (
...
function MatchRow({ item, tab }: { item: any; tab: Tab }) {
  const name = item.other_name || item.full_name || item.worker_name || item.title || 'Εργαζόμενος';
  const subtitle = item.role || item.job_title || item.city || '';
  const photo = item.other_photo || item.other_avatar || item.avatar_url || item.photo_url;
  const href = item.conversation_id
    ? `/app2/version7/business/chat?id=${item.conversation_id}`
    : `/app2/version7/business/worker?id=${item.worker_id || item.user_id || item.id}`;
