...
/**
 * Worker profile — merged v5 hero (gradient + stats + XP + completeness +
 * badges + "Δες το προφίλ σου" CTA) with v7 settings rows below.
 */
...
import { auth, clearToken, stats as statsApi, workers, type User } from '../../_lib/api';
...
export default function WorkerProfile() {
    ...
    (async () => {
      try {
        const [me, w, s] = await Promise.all([
          auth.me(),
          workers.me().catch(() => null),
          statsApi.dashboard().catch(() => null),
        ]);
        if (cancelled) return;
        setUser(me.user);
        setProfile(w?.profile || me.profile || null);
        setRoles(w?.roles || []);
        if (s) setStats(s);
  ...
  const name = profile?.full_name || user?.display_name || user?.email?.split('@')[0] || 'Χρήστης';
  const initials = name.split(' ').slice(0, 2).map((n: string) => n[0]?.toUpperCase()).join('') || '?';
  const photo = profile?.photo_url || profile?.avatar_url || user?.avatar_url;
  const completeness = profile?.profile_completeness || profile?.completion_pct || 40;
  const role = roles[0] ? humaniseRole(roles[0]) : profile?.title || 'Εργαζόμενος';
  const streak = (() => {
    // deterministic-ish from user id so it doesn't flicker between renders
    const seed = (user?.id || '').split('').reduce((a, c) => a + c.charCodeAt(0), 0);
    return (seed % 7) + 1;
  })();

  // XP / level (mirrored from v5)
  const xp = (completeness * 5) + ((stats.total_matches || 0) * 30) + ((stats.pending_interests || 0) * 10);
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 overflow-y-auto">
      {/* ─────────── HERO (v5) ─────────── */}
      <div
        className="flex-shrink-0 bg-gradient-to-br from-purple-600 via-pink-500 to-red-500 px-4 pt-8 pb-20 text-center text-white relative"
        style={{ paddingTop: 'calc(32px + env(safe-area-inset-top))' }}
      >
        <Link
          href="/app2/version7/worker/settings"
        ...
        <p className="text-sm text-white/90">
          {role}{profile?.city ? ` · 📍 ${profile.city}` : ''}
        </p>
      ...
      {/* ─────────── Cards (v5: XP / completeness / badges / preview CTA) ─────────── */}
      <div className="px-4 mt-6 space-y-4 pb-4">
        ...
        {/* Completeness */}
        <Link href="/app2/version7/worker/profile/edit" className="block rounded-2xl bg-white p-5 shadow-sm">
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {streak >= 3 && <Badge tone="orange">🔥 {streak}-day Streak</Badge>}
            {profile?.verified === 1 && <Badge tone="emerald">✓ Verified</Badge>}
        ...
        <Link
          href={user?.id ? `/app2/version7/worker/preview?id=${user.id}` : '#'}
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 text-left">
              ...
              <p className="text-xs text-white/80">Όπως το βλέπουν οι επιχειρήσεις</p>
            </div>
      ...
      <div className="px-4 pb-8 space-y-4">
        <Section title="Ρυθμίσεις">
          <FieldGroup>
            <Row icon="✏️" label="Επεξεργασία προφίλ" hint="Όνομα, φωτογραφία, ειδικότητες" href="/app2/version7/worker/profile/edit" />
            <Row icon="📄" iconBg="bg-emerald-50" iconColor="text-emerald-600" label="Βιογραφικό (CV)" hint="Δημιούργησε & κατέβασε PDF" href="/app2/version7/worker/cv" />
            <Row icon="⭐" iconBg="bg-amber-50" iconColor="text-amber-600" label="Premium" hint="Ξεχώρισε με αστέρι & boosts" href="/app2/version7/worker/billing" />
            <Row icon="🔔" iconBg="bg-cyan-50" iconColor="text-cyan-600" label="Ειδοποιήσεις" href="/app2/version7/worker/notifications" />
            <Row icon="⚙️" iconBg="bg-gray-100" iconColor="text-gray-700" label="Γενικές ρυθμίσεις" href="/app2/version7/worker/settings" last />
          </FieldGroup>
        </Section>
...
function humaniseRole(id: string): string {
