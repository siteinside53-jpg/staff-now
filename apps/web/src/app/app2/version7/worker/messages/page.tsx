...
export default function WorkerMessages() {
  ...
  const filtered = search
    ? items.filter((c) =>
        ((c.other_user_name || c.business_name || c.worker_name || '') + ' ' + (c.last_message || ''))
          .toLowerCase()
    ...
    <>
      ...
      <Body>
        {loading ? (
          ...
          <EmptyState icon="💬" title="Καμία συνομιλία" description="Όταν κάνεις match, θα εμφανιστούν εδώ τα μηνύματα." />
        ) : (
...
function ConvRow({ conv }: { conv: any }) {
  ...
  return (
    <Card href={`/app2/version7/worker/chat?id=${conv.id}`} className="p-3">
      <div className="flex items-center gap-3">
        <Avatar name={name} src={conv.other_photo || conv.other_avatar || conv.other_user_avatar || conv.business_logo || conv.worker_avatar} size="md" />
