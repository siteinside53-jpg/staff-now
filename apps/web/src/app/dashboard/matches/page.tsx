...
export default function MatchesPage() {
  ...
  const likeBack = async (item: any) => {
    ...
    try {
      ...
      if (data.success && data.data?.matched) {
        toast.success(t('matchesPage.toasts.matched'));
        setRequests((prev) => prev.map((i) => i.swiper_id === item.swiper_id
          ? { ...i, is_matched: 1, conversation_id: data.data.conversationId }
          : i));
      } else {
        toast.error(data.error?.message || t('matchesPage.toasts.generic'));
      }
    } catch { toast.error(t('matchesPage.toasts.generic')); }
  ...
  const matchAction = async (matchId: string, action: 'archive' | 'restore' | 'delete') => {
    ...
    try {
      if (action === 'archive') {
        ...
        toast.success(t('matchesPage.toasts.archived'));
        ...
        if (data.success) {
          ...
          toast.success(t('matchesPage.toasts.restored'));
        } else { toast.error(data.error?.message || t('matchesPage.toasts.error')); }
      } else if (action === 'delete') {
        if (!confirm(t('matchesPage.confirmDelete'))) return;
        const m = matches.find((x) => x.id === matchId);
        // 1) Permanently remove the match (server-side).
        const res = await fetch(`${base}/matches/${matchId}`, { method: 'DELETE', headers });
        const data = (await res.json()) as any;
        if (!data.success) {
          toast.error(data.error?.message || t('matchesPage.toasts.deleteFailed'));
          return;
        }
        ...
        toast.success(t('matchesPage.toasts.deleted'));
      }
    } catch { toast.error(t('matchesPage.toasts.error')); }
    setMenuId(null);
  };
  ...
  const nameOf = (m: any) => isWorker
    ? (m.business_name || m.company_name || t('matchesPage.business'))
    : (m.worker_name || m.full_name || t('matchesPage.worker'));
        ...
        */}
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <h1 className="text-2xl font-extrabold sm:text-3xl">{t('matchesPage.title')}</h1>
            <p className="mt-1 text-sm text-blue-100">
              {matches.length > 0
                ? t('matchesPage.summary', { active: activeCount, total: matches.length })
                : t('matchesPage.none')}
            </p>
          </div>
          <Link
      ...
      */}
      <div className="mb-6 flex items-center gap-2">
        <div className="flex min-w-0 flex-1 rounded-full bg-gray-100 p-1">
          {[
            { key: 'active' as const, icon: '✨', label: t('matchesPage.tabs.matches'), count: activeCount },
            { key: 'requests' as const, icon: '👋', label: t('matchesPage.tabs.requests'), count: pendingRequests.length },
            ...(tab === 'archived' ? [{ key: 'archived' as const, icon: '📦', label: t('matchesPage.tabs.archive'), count: archivedCount }] : []),
            ...(tab === 'blocked' ? [{ key: 'blocked' as const, icon: '🚫', label: t('matchesPage.tabs.blocked'), count: blockedCount }] : []),
          ].map((t) => (
            ...
            <>
              ...
              <div className="absolute right-0 top-12 z-20 w-48 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
                <button onClick={() => { setTab('archived'); setMenuId(null); }}
                  ...
                  {t('matchesPage.tabs.archive')}{archivedCount > 0 && <span className="ml-auto text-xs text-gray-400">{archivedCount}</span>}
                </button>
                <button onClick={() => { setTab('blocked'); setMenuId(null); }}
                  ...
                  {t('matchesPage.tabs.blocked')}{blockedCount > 0 && <span className="ml-auto text-xs text-gray-400">{blockedCount}</span>}
                </button>
              </div>
            </>
          )}
      ...
      {tab === 'requests' ? (
        pendingRequests.length === 0 ? (
          <EmptyState
            title={t('matchesPage.requestsEmpty')}
            description={isWorker
              ? t('matchesPage.requestsEmptyWorker')
              : t('matchesPage.requestsEmptyBusiness')}
          />
        ) : (
          <ul className="space-y-1">
            {pendingRequests.map((r: any) => {
              const name = isWorker
                ? (r.company_name || t('matchesPage.business'))
                : (r.full_name || t('matchesPage.worker'));
              ...
              const subtitle = [
                ...
                r.liked_at ? new Date(r.liked_at).toLocaleDateString(locale === 'en' ? 'en-GB' : 'el-GR', { day: 'numeric', month: 'short' }) : null,
              ].filter(Boolean).join(' · ');
              ...
              return (
                <li key={r.swipe_id} className="flex items-center gap-2">
                  ...
                  >
                    {liking === r.swiper_id ? '…' : t('matchesPage.interested')}
                  </button>
                </li>
              );
            })}
        ...
        <EmptyState
          title={tab === 'active' ? t('matchesPage.emptyActive') : tab === 'archived' ? t('matchesPage.emptyArchived') : t('matchesPage.emptyBlocked')}
          description={tab === 'active' ? t('matchesPage.emptyActiveDesc') : ''}
        />
      ) : (
        <>
          {/*
          ...
          */}
          {tab === 'active' && recent.length >= 2 && (
            <div className="mb-6">
              <h2 className="mb-3 text-xs font-bold uppercase tracking-wider text-gray-400">{t('matchesPage.newMatches')}</h2>
          ...
          <ul className="space-y-1">
            {filtered.map((m: any) => {
              ...
              const subtitle = [
                ...
                matchDate ? new Date(matchDate).toLocaleDateString(locale === 'en' ? 'en-GB' : 'el-GR', { day: 'numeric', month: 'short' }) : null,
              ].filter(Boolean).join(' · ');
                  ...
                  <button onClick={() => openProfile(m)} className="flex min-w-0 flex-1 items-center gap-3 rounded-lg py-3 text-left hover:bg-gray-50 active:bg-gray-50">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-1.5">
                        {m.isBlocked ? (
                          <Badge className="bg-red-100 text-red-700 border-red-200">{t('matchesPage.blockedBadge')}</Badge>
                        ) : m.status === 'archived' ? (
                          <Badge variant="secondary">{t('matchesPage.archiveBadge')}</Badge>
                        ) : null}
                  ...
                  */}
                  <div className={`relative flex-shrink-0 ${menuId === m.id ? 'z-30' : ''}`}>
                    <button onClick={() => setMenuId(menuId === m.id ? null : m.id)}
                      aria-label={t('matchesPage.options')}
