...
export default function JobsPage() {
  ...
  const addPosition = () => {
    if (positions.length >= 10) { toast.error(t('jobsPage.maxPositions')); return; }
  ...
  const handleCreate = async () => {
    if (!form.title) { toast.error(t('jobsPage.errTitle')); return; }
    if (!form.city) { toast.error(t('jobsPage.errCity')); return; }
    if (!form.branchId && branches.length > 0) { toast.error(t('jobsPage.errBranch')); return; }
    ...
    if (!hasBenefit) { toast.error(t('jobsPage.errBenefit')); return; }
    ...
    try {
      ...
      toast.success(t('jobsPage.published'));
      setShowForm(false);
      setForm({ ...EMPTY_FORM });
      await fetchData();
    } catch (err: any) {
      if (err?.code === 'JOB_LIMIT_REACHED') {
        ...
        toast.error(t('jobsPage.createFailed'));
      }
    } finally { setSaving(false); }
  };

  if (user?.role === 'worker') return (
    <div><h1 className="mb-4 text-2xl font-bold text-gray-900">{t('jobsPage.title')}</h1><p className="text-gray-600">{t('jobsPage.businessOnly')}</p></div>
  );
  ...
  const statusLabels: Record<string, string> = {
    ...
    filled: t('jobsPage.statusFilled'),
  };
  ...
  const handlePauseResume = async (jobId: string, currentStatus: string) => {
    try {
      if (currentStatus === 'published') { await api.jobs.pause(jobId); toast.success(t('jobsPage.pausedToast')); }
      else if (currentStatus === 'paused') { await api.jobs.resume(jobId); toast.success(t('jobsPage.resumedToast')); }
      await fetchData();
    } catch { toast.error(t('jobsPage.updateError')); }
  };
  ...
  const handleFillToggle = async (jobId: string, currentStatus: string) => {
    try {
      if (currentStatus === 'filled') {
        ...
        toast.success(t('jobsPage.reopened'));
      } else {
        if (!confirm(t('jobsPage.confirmFill'))) return;
        await (api.jobs as any).fill(jobId);
        toast.success(t('jobsPage.filledToast'));
      }
      await fetchData();
    } catch { toast.error(t('jobsPage.updateError')); }
  };
  ...
  const handleDelete = async (jobId: string) => {
    if (!confirm(t('jobsPage.confirmDelete'))) return;
    try { await api.jobs.delete(jobId); toast.success(t('jobsPage.deleted')); await fetchData(); } catch { toast.error(t('jobsPage.deleteError')); }
  };
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('jobsPage.title')}</h1>
          <p className="mt-1 text-gray-600">{jobs.length > 0 ? t('jobsPage.countJobs', { count: jobs.length }) : t('jobsPage.noJobs')}</p>
        </div>
      ...
      {showForm && (
        <div className="mb-8 max-w-3xl space-y-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">{t('jobsPage.newJobTitle')}</h2>
              <p className="text-sm text-gray-500">{t('jobsPage.newJobSubtitle')}</p>
            </div>
          </div>
          ...
          {branches.length === 0 && (
            <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 text-sm text-amber-800">
              {t('jobsPage.addBusinessFirst')} <a href="/dashboard/profile" className="font-semibold underline">{t('jobsPage.profileLink')}</a>.
            </div>
          )}
          ...
          <Card><CardHeader><h3 className="text-base font-bold text-gray-900"><span className="text-blue-600 mr-2">1.</span>{t('jobsPage.secBasic')}</h3></CardHeader>
            <CardContent className="space-y-4">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.jobTitle')}</label>
                <Input value={form.title} onChange={(e) => f('title', e.target.value)} placeholder={t('jobsPage.jobTitlePlaceholder')} />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.description')} <span className="text-gray-400 text-xs">({form.description.length}/300)</span></label>
                <Textarea value={form.description} onChange={(e) => { if (e.target.value.length <= 300) f('description', e.target.value); }} rows={3} placeholder={t('jobsPage.descriptionPlaceholder')} />
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.address')}</label>
                  <Input value={form.address} onChange={(e) => f('address', e.target.value)} placeholder={t('jobsPage.addressPlaceholder')} /></div>
          ...
          <Card><CardHeader><h3 className="text-base font-bold text-gray-900"><span className="text-blue-600 mr-2">4.</span>{t('jobsPage.secSalary')}</h3></CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.salaryFrom')}</label>
                  <Input type="number" min="0" value={form.salaryMin} onChange={(e) => f('salaryMin', e.target.value)} placeholder="1200" /></div>
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.salaryTo')}</label>
          ...
          {/* 5. Παροχές */}
          <Card><CardHeader><h3 className="text-base font-bold text-gray-900"><span className="text-blue-600 mr-2">5.</span>{t('jobsPage.secBenefits')} <span className="text-xs text-gray-400 font-normal">{t('jobsPage.atLeastOne')}</span></h3></CardHeader>
            <CardContent>
              <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  return (
                    <div key={item.key} onClick={() => toggleBenefit(item.key)}
                      ...
                      <div className={`mt-2 inline-block rounded-full px-3 py-0.5 text-xs font-medium ${on ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                        {on ? t('jobsPage.yes') : t('jobsPage.no')}
                      </div>
                    </div>
                  );
                })}
          ...
          <Card><CardHeader><h3 className="text-base font-bold text-gray-900"><span className="text-blue-600 mr-2">6.</span>{t('jobsPage.secHours')}</h3></CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.hoursPerDay')}</label>
                  <Input type="number" min="1" max="24" value={form.hoursPerDay} onChange={(e) => f('hoursPerDay', e.target.value)} placeholder="8" /></div>
                <div><label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.daysPerWeek')}</label>
              ...
              <div>
                ...
                {form.hasDayOff && (
                  <Input value={form.dayOffDescription} onChange={(e) => f('dayOffDescription', e.target.value)} placeholder={t('jobsPage.dayOffPlaceholder')} className="mt-2" />
                )}
              </div>
              <div><label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.shift')}</label>
                <select value={form.shiftType} onChange={(e) => f('shiftType', e.target.value)} className={sel}>
                  <option value="">{t('jobsPage.choose')}</option>
                  {Object.keys(SHIFT_TYPE_LABELS_EL).map((v) => <option key={v} value={v}>{t(`jobsPage.shift${v.charAt(0).toUpperCase()}${v.slice(1)}`)}</option>)}
                </select></div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div><label className="mb-1.5 block text-sm font-medium text-gray-700">{t('jobsPage.experience')}</label>
                <select value={form.experienceRequired} onChange={(e) => f('experienceRequired', e.target.value)} className={sel}>
                  <option value="">{t('jobsPage.choose')}</option>
                  {Object.keys(EXPERIENCE_LABELS_EL).map((v) => <option key={v} value={v}>{v === 'none' ? t('jobsPage.expNone') : v === '1_2_years' ? t('jobsPage.exp1to2') : t('jobsPage.exp3plus')}</option>)}
                </select></div>
          ...
          {/* 10. Ειδικότητες */}
          <Card><CardHeader><h3 className="text-base font-bold text-gray-900"><span className="text-blue-600 mr-2">10.</span>{t('jobsPage.secRolesWanted')}</h3></CardHeader>
            <CardContent className="space-y-4">
              {positions.map((pos, idx) => (
                <div key={idx} className="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold text-gray-700">{t('jobsPage.positionN', { n: idx + 1 })}</span>
                    {positions.length > 1 && (
                      <button onClick={() => removePosition(idx)} className="text-xs text-red-500 hover:text-red-700">{t('jobsPage.remove')}</button>
                    )}
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div><label className="mb-1 block text-xs font-medium text-gray-600">{t('jobsPage.role')}</label>
                      <select value={pos.role} onChange={(e) => updatePosition(idx, 'role', e.target.value)} className={sel}>
                        <option value="">{t('jobsPage.choose')}</option>
                        {Object.keys(WORKER_JOB_ROLE_LABELS_EL).map((v) => <option key={v} value={v}>{labels.role(v)}</option>)}
                      </select></div>
                    <div><label className="mb-1 block text-xs font-medium text-gray-600">{t('jobsPage.people')}</label>
                  ...
                  <div><label className="mb-1 block text-xs font-medium text-gray-600">{t('jobsPage.positionNotes')}</label>
                    <Textarea value={pos.description} onChange={(e) => updatePosition(idx, 'description', e.target.value)} rows={2} placeholder={t('jobsPage.positionNotesPlaceholder')} className="text-sm" /></div>
                </div>
              ))}
          ...
          <Button onClick={handleCreate} disabled={saving} size="lg" className="w-full">
            {saving ? t('jobsPage.publishing') : t('jobsPage.publishJob')}
          </Button>
        </div>
      )}
      ...
      {jobs.length === 0 && !showForm ? (
        <EmptyState title={t('jobsPage.emptyTitle')} description={t('jobsPage.emptyDesc')} />
      ) : !showForm && (
                  ...
                  <div className="min-w-0 w-full sm:w-auto sm:flex-1">
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
                      ...
                      */}
                      {((Number(job.hires_confirmed) || 0) > 0 || hireTarget(job) > 1) && (
                        ...
                        >
                          {t('jobsPage.hiresCount', { done: Number(job.hires_confirmed) || 0, target: hireTarget(job) })}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-3 text-sm text-gray-400">
                      {job.listing_kind === 'shift' && job.shift_date && (
                        <span className="font-semibold text-red-600">
                          ...
                          {(job.shift_positions ?? 1) > 1 ? ` · ${t('jobsPage.peopleCount', { count: job.shift_positions })}` : ''}
                        </span>
                      )}
                      {job.city && <span>📍 {job.city}{job.region ? `, ${job.region}` : ''}</span>}
                      {(job.salary_min || job.salary_max) && (() => {
                        const unit = job.salary_type === 'hourly' ? t('jobsPage.perHour') : job.salary_type === 'daily' ? t('jobsPage.perDay') : t('jobsPage.perMonth');
                        const amount = job.salary_min && job.salary_max ? `${job.salary_min}-${job.salary_max}€` : job.salary_min ? t('jobsPage.fromAmount', { amount: job.salary_min }) : t('jobsPage.toAmount', { amount: job.salary_max });
                    ...
                    {(job.housing_provided === 1 || job.meals_provided === 1 || job.transport_provided === 1 || job.bonus_provided === 1 || job.insurance_provided === 1) && (
                      <div className="mt-2 flex flex-wrap gap-2">
                        {job.housing_provided === 1 && <span className="rounded-full bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-xs font-medium text-emerald-700">{t('jobsPage.bHousing')}</span>}
                        {job.meals_provided === 1 && <span className="rounded-full bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-xs font-medium text-emerald-700">{t('jobsPage.bMeals')}</span>}
                        {job.transport_provided === 1 && <span className="rounded-full bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-xs font-medium text-emerald-700">{t('jobsPage.bTransport')}</span>}
                        {job.bonus_provided === 1 && <span className="rounded-full bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-xs font-medium text-emerald-700">{t('jobsPage.bBonus')}</span>}
                        {job.insurance_provided === 1 && <span className="rounded-full bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-xs font-medium text-emerald-700">{t('jobsPage.bFlexible')}</span>}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="ml-auto flex min-w-0 flex-wrap justify-end gap-1.5">
                    {(job.status === 'published' || job.status === 'paused') && (
                      <button onClick={() => handlePauseResume(job.id, job.status)}
                        title={job.status === 'published' ? t('jobsPage.pause') : t('jobsPage.resume')}
                    ...
                    <button onClick={() => setPreviewJobId(job.id)} title={t('jobsPage.preview')} className="rounded-lg border border-gray-200 p-2 hover:bg-gray-50 text-gray-400 hover:text-emerald-600 transition-colors">
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                    </button>
                    <a href={`/dashboard/jobs/edit?id=${job.id}`} title={t('jobsPage.edit')} className="rounded-lg border border-gray-200 p-2 hover:bg-gray-50 text-gray-400 hover:text-blue-600 transition-colors">
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                    </a>
                    <button onClick={() => handleDelete(job.id)} title={t('jobsPage.delete')} className="rounded-lg border border-gray-200 p-2 hover:bg-red-50 text-gray-400 hover:text-red-600 transition-colors">
