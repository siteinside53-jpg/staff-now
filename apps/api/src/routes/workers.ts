...
workers.get('/discover', requireAuth, requireRole('business'), async (c) => {
  ...
  const dataQuery = `
    SELECT DISTINCT wp.*, u.email, u.status as user_status, u.is_premium,
      ...
      -- Πόσοι διαφορετικοί επισκέπτες άνοιξαν το προφίλ σήμερα. Πραγματικός
      -- αριθμός από το profile_views (γράφεται στο GET /workers/:id). Χρησιμοποιεί
      -- το index idx_profile_views_worker. Αν κανείς δεν το είδε → 0 και η κάρτα
      -- δεν δείχνει καθόλου το ματάκι.
      (SELECT COUNT(DISTINCT viewer_id) FROM profile_views pv
        WHERE pv.worker_id = wp.user_id AND date(pv.created_at) = date('now')) as views_today
