...
/*
  ΠΩΣ ΚΡΥΒΕΤΑΙ Η ΘΕΣΗ ΑΠΟ ΤΟ ΚΟΙΝΟ — ΚΑΙ ΓΙΑΤΙ ΟΧΙ ΜΕ ΜΕΤΑΤΟΠΙΣΗ.

  ΤΟ ΛΑΘΟΣ ΠΟΥ ΔΙΟΡΘΩΝΕΤΑΙ ΕΔΩ: η πρώτη εκδοχή μετατόπιζε το σημείο ~300 μέτρα
  προς τυχαία κατεύθυνση. Η κατεύθυνση όμως έβγαινε από το id της αγγελίας — που
  είναι ΔΗΜΟΣΙΟ — με απλό, αναγνωρίσιμο υπολογισμό. Οποιοσδήποτε ήξερε τον
  υπολογισμό αφαιρούσε το ίδιο ακριβώς διάνυσμα και έπαιρνε πίσω τη διεύθυνση.
  Το μέτρησα: σφάλμα ανάκτησης 0,12 ΜΕΤΡΑ. Δηλαδή μηδενική προστασία, με την
  ψευδαίσθηση της προστασίας — που είναι χειρότερο από καθόλου, γιατί ο χρήστης
  δίνει τη διεύθυνσή του νομίζοντας ότι δίνει περιοχή.

  ΤΙ ΓΙΝΕΤΑΙ ΤΩΡΑ: το σημείο ΚΟΥΜΠΩΝΕΙ σε πλέγμα. Όλες οι διευθύνσεις μέσα στο
  ίδιο κελί βγάζουν ΤΟ ΙΔΙΟ δημόσιο σημείο. Είναι πράξη πολλά-προς-ένα: δεν
  υπάρχει τίποτα να αφαιρέσεις, όσο κι αν ξέρεις τον υπολογισμό. Το μόνο που
  μαθαίνει κανείς είναι «κάπου μέσα σε αυτό το τετράγωνο».

  ΚΑΙ ΔΕΝ ΣΠΑΕΙ ΜΕ ΠΟΛΛΕΣ ΑΓΓΕΛΙΕΣ: δέκα αγγελίες από το ίδιο σπίτι δίνουν δέκα
  ΦΟΡΕΣ το ίδιο σημείο, όχι δέκα κύκλους που τέμνονται. Η τυχαία μετατόπιση
  έσπαγε ακριβώς έτσι.
*/
const CELL_LAT = 0.005; // ~555 μέτρα
const CELL_LON = 0.007; // ~590 μέτρα στο γεωγραφικό πλάτος της Ελλάδας

function snapPoint(lat: number, lon: number): { lat: number; lon: number } {
  return {
    lat: +(Math.round(lat / CELL_LAT) * CELL_LAT).toFixed(4),
    lon: +(Math.round(lon / CELL_LON) * CELL_LON).toFixed(4),
  };
}

/** Ανοιχτή σημαίνει: δέχεται προσφορές τώρα. *//** Ανοιχτή σημαίνει: δέχεται προσφορές τώρα. */
    ...
    */
    ...(opts.point
      ? (() => {
          ...
          const p = exact ? opts.point : snapPoint(opts.point.lat, opts.point.lon);
...
/*
  ...
  ΧΩΡΙΣ ΣΥΝΔΕΣΗ, ΕΠΙΤΗΔΕΣ: η ροή των μικροδουλειών είναι δημόσια και τη βλέπει
  ο καθένας. Το «γράψε τη διεύθυνσή σου» είναι η ΜΟΝΗ διέξοδος για όποιον ο
  browser του αρνείται την τοποθεσία — και μια άρνηση δεν ξαναρωτιέται ποτέ.
  Με σύνδεση, ο επισκέπτης χωρίς λογαριασμό έμενε κλειδωμένος στο «Όλη η
  Θεσσαλονίκη» χωρίς κανέναν τρόπο να δει τι υπάρχει κοντά του.

  Δεν εκτίθεται τίποτα δικό μας: γυρίζει δημόσια δεδομένα χάρτη, το πολύ 5
  αποτελέσματα, μόνο Ελλάδα, και οι απαντήσεις είναι αποθηκευμένες. Ισχύει
  κανονικά το φρένο των 120 αιτημάτων ανά λεπτό ανά διεύθυνση δικτύου.
*/
/*
  GET /where-am-i — ΚΑΤΑ ΠΡΟΣΕΓΓΙΣΗ ΤΟΠΟΘΕΣΙΑ, ΧΩΡΙΣ ΚΑΜΙΑ ΑΔΕΙΑ.

  ΤΟ ΠΡΟΒΛΗΜΑ ΠΟΥ ΛΥΝΕΙ: το «Κοντά μου» στηριζόταν αποκλειστικά στον browser.
  Όταν εκείνος αρνιόταν — και στο Safari σε Mac αρνείται ΑΚΟΜΗ ΚΑΙ με όλες τις
  άδειες δοσμένες — ο χρήστης έμενε να παλεύει με ρυθμίσεις συστήματος αντί να
  δει δουλειές κοντά του. Καμία σελίδα δεν μπορεί να αναγκάσει το λειτουργικό
  να δώσει τοποθεσία, οπότε χρειαζόταν ΔΕΥΤΕΡΟΣ δρόμος.

  ΑΠΟ ΠΟΥ ΒΓΑΙΝΕΙ: από το ίδιο το αίτημα που μόλις έκανε ο χρήστης. Το δίκτυο
  της Cloudflare, που ήδη σερβίρει κάθε κλήση μας, ξέρει κατά προσέγγιση από
  πού ήρθε. Δεν ρωτάμε τρίτη υπηρεσία, δεν στέλνουμε τίποτα πουθενά, δεν
  αποθηκεύουμε τίποτα.

  ΤΙ ΑΚΡΙΒΕΙΑ ΕΧΕΙ: επιπέδου πόλης — μερικά χιλιόμετρα. ΔΕΝ είναι η διεύθυνσή
  σου και δεν παρουσιάζεται ποτέ ως τέτοια. Για τις μικροδουλειές αρκεί: το
...
tasknow.get('/where-am-i', async (c) => {
  ...
  // Εκτός Ελλάδας δεν έχει νόημα: η ροή είναι ελληνική και ένα κέντρο στο
  // Βερολίνο θα έκρυβε τα πάντα αντί να βοηθήσει.
  const inGreece = cf?.country === 'GR';

  if (!Number.isFinite(lat) || !Number.isFinite(lon) || !inGreece) {
